...
import { Repository, DeepPartial } from 'typeorm'; // Importa Repository y DeepPartial
import { PermissionEntity } from 'src/entities/permissions.entity';
import { RoleEntity } from 'src/entities/roles.entity';

@Injectable()
export class RolesService {
  // 1. CORRECCIÓN: Inyección de repositorios
  constructor(
    @InjectRepository(RoleEntity)
    private readonly roleRepository: Repository<RoleEntity>, // Inyecta RoleEntity
    @InjectRepository(PermissionEntity)
    private readonly permissionRepository: Repository<PermissionEntity>, // Inyecta PermissionEntity
  ) {}
  ...
  async createRole(role: DeepPartial<RoleEntity>): Promise<RoleEntity> {
    try {
      ...
      throw new BadRequestException(
        'Error al crear el rol. Verifique los datos.',
      );
    }
  }
  ...
  async actualizarRole(
    ...
    const roleAActualizar = await this.roleRepository.findOneBy({
      id: param.id,
    });

    if (!roleAActualizar) {
      throw new NotFoundException(
        'No se encontró ningún rol con el ID ingresado',
      );
    }
    ...
    if (roleDto.descripcion !== undefined) {
      roleAActualizar.descripcion = roleDto.descripcion;
    }
  ...
  async deleteRole(id: number): Promise<{ message: string }> {
    ...
    if (!roleAEliminar) {
      throw new NotFoundException(
        `No se encontró ningún rol con el ID ${id} para eliminar.`,
      );
    }
